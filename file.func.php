...
//验证文件名是否合法
	if(checkFilename($newname)){
		//检测当前目录下是否存在同名文件
		$path=dirname($oldname);//得到路径
		if(!file_exists($path."/".$newname)){//判断文件是否存在
			//进行重命名
...
function downFile($filename){
	header("content-disposition:attachment;filename=".basename($filename));//nasemane得到主文件名
 ...
 * @return string
 */
function uploadFile($fileInfo,$path,$allowExt=array("gif","jpeg","jpg","png","txt","html","php","js","css"),$maxSize=10485760){
	//判断错误号
