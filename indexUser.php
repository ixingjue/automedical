...
if(!$info){
	echo "<script>alert('没有文件或目录！！！');location.href='indexUser.php';</script>";
}
//print_r($info);
$redirect="indexUser.php?path={$path}";
if($act=="创建文件"){
	//创建文件
//	echo $path,"--";
//	echo $filename;
...
EOF;
	...
	//echo $content;
	$str=<<<EOF
	<form action='indexUser.php?act=doEdit' method='post'> 
...
EOF;
	...
	//完成重命名
	$str=<<<EOF
	<form action="indexUser.php?act=doRename" method="post"> 
...
echo $str;
	...
	$str=<<<EOF
			<form action="indexUser.php?act=doRenameFolder" method="post"> 
...
echo $str;
	...
	alertMes($mes,$redirect);
		...
		$str=<<<EOF
	<form action="indexUser.php?act=doCopyFolder" method="post"> 
...
echo $str;
	...
	alertMes($mes,$redirect);
			...
			$str=<<<EOF
	<form action="indexUser.php?act=doCutFolder" method="post"> 
...
echo $str;
	...
	alertMes($mes,$redirect);
				...
				$str=<<<EOF
	<form action="indexUser.php?act=doCopyFile" method="post"> 
...
echo $str;
	...
	alertMes($mes,$redirect);
				...
				$str=<<<EOF
	<form action="indexUser.php?act=doCutFile" method="post"> 
...
<script type="text/javascript">
	...
	function delFile(filename,path){
		if(window.confirm("您确定要删除嘛?删除之后无法恢复哟!!!")){
				location.href="indexUser.php?act=delFile&filename="+filename+"&path="+path;
		}
	}
	function delFolder(dirname,path){
		if(window.confirm("您确定要删除嘛?删除之后无法恢复哟!!!")){
			location.href="indexUser.php?act=delFolder&dirname="+dirname+"&path="+path;
		}
	}
	...
	function goBack($back){
		location.href="indexUser.php?path="+$back;
	}
</script>
</head>
...
<div id="top">
	<ul id="navi">
		<li><a href="indexUser.php" title="主目录"><span style="margin-left: 8px; margin-top: 0px; top: 4px;" class="icon icon-small icon-square"><span class="icon-home"></span></span></a></li>
...
<form action="indexUser.php" method="post" enctype="multipart/form-data">
...
<tr id="createFolder"  style="display:none;">
	...
	<tr>
		...
		<td class="size">大小</td>
		<td class="read">可读</td>
		<td class="write">可写</td>
		<td class="zhixing">可执行</td>
		<td class="creatt">创建时间</td>
		<td class="ret">修改时间</td>
		<td class="visitt">访问时间</td>
		<td class="do">操作</td>
	</tr>
<?php 
if($info['file']){
...
?>
	<tr>
		...
		?>
		<a href="indexUser.php?act=showContent&path=<?php echo $path;?>&filename=<?php echo $p;?>" ><img class="small" src="images/show.png"  alt="" title="查看"/></a>|
		<?php }?>
		<a href="indexUser.php?act=editContent&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/edit.png"  alt="" title="修改"/></a>|
		<a href="indexUser.php?act=renameFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/rename.png"  alt="" title="重命名"/></a>|
		<a href="indexUser.php?act=copyFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/copy.png"  alt="" title="复制"/></a>|
		<a href="indexUser.php?act=cutFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small" src="images/cut.png"  alt="" title="剪切"/></a>|
		<a href="#"  onclick="delFile('<?php echo $p;?>','<?php echo $path;?>')"><img class="small" src="images/delete.png"  alt="" title="删除"/></a>|
		<a href="indexUser.php?act=downFile&path=<?php echo $path;?>&filename=<?php echo $p;?>"><img class="small"  src="images/download.png"  alt="" title="下载"/></a>
		</td>		
	</tr>
<?php 
$i++;
...
?>
	<tr>
		...
		<td><?php echo date("Y-m-d H:i:s",fileatime($p));?></td>
		<td>
		<a href="indexUser.php?path=<?php echo $p;?>" ><img class="small" src="images/show.png"  alt="" title="查看"/></a>|
		<a href="indexUser.php?act=renameFolder&path=<?php echo $path;?>&dirname=<?php echo $p;?>"><img class="small" src="images/rename.png"  alt="" title="重命名"/></a>|
		<a href="indexUser.php?act=copyFolder&path=<?php echo $path;?>&dirname=<?php echo $p;?>"><img class="small" src="images/copy.png"  alt="" title="复制"/></a>|
		<a href="indexUser.php?act=cutFolder&path=<?php echo $path;?>&dirname=<?php echo $p;?>"><img class="small" src="images/cut.png"  alt="" title="剪切"/></a>|
