...
class Readability
{
    ...
    private function getTopBox()
    {
        ...
        while ($paragraph = $allParagraphs->item($i ++)) {
            ...
            if (preg_match("/(comment|meta|footer|footnote)/i", $className)) {
                ...
                if (preg_match("/((^|\\s)(post|hentry|entry[-]?(content|text|body)?|article[-]?(content|text|body)?)(\\s|$))/i", $className)) {
                    $contentScore += 25;
                }
            ...
            if (preg_match("/(comment|meta|footer|footnote)/i", $id)) {
                ...
                if (preg_match("/^(post|hentry|entry[-]?(content|text|body)?|article[-]?(content|text|body)?)$/i", $id)) {
                    $contentScore += 25;
                }
    ...
    {   
        ...
        if (! $this->DOM)
            ...
            // 获取页面标题
        $ContentTitle = trim($this->getTitle());
        
            for ($i = 0; $i < count($str_ay); $i ++) {
                                if (strpos($ContentTitle, $str_ay[$i]) > 0) {                   
                    $ContentTitle = trim(substr($ContentTitle, 0, strpos($ContentTitle, $str_ay[$i])));
                    $flag=true;
                }
          
        }             

        if (strrchr($ContentTitle, ' ')&&(!$flag)&&(strpos($ContentTitle,'网') > 0)) {
            $ContentTitle = str_replace(strrchr($ContentTitle, ' '), '', $ContentTitle);
        }
